...
class PPC_meta_boxes {
    ...
    */
    ...
    static function meta_box_misc_settings( $post, $current_settings ) {
        ...
        echo PPC_HTML_functions::echo_p_field( 'This custom number of days', $current_settings['default_stats_time_range_custom'], 'radio', 'default_stats_time_range', __( 'You can manually customize the rime range for the posts that will be displayed. So, for example, if you set this to 365 days, in the stats page it will automatically be selected a time frame that goes from the current day to the previous 365 days.' , 'post-pay-counter'), 'default_stats_time_range_custom', 'default_stats_time_range_custom' );
    ...
    */
    ...
    static function meta_box_counting_settings( $post, $current_settings ) {
        ...
        echo PPC_HTML_functions::echo_p_field( 'Exclude quoted content from word counting', $current_settings['counting_exclude_quotations'], 'checkbox', 'counting_exclude_quotations', sprintf( __( 'All the words contained into %1$sblockquote%2$s tags will not be taken into account when counting. Use this to prevent interviews and such stuff to be counted as normal words.' , 'post-pay-counter'), '<em>', '</em>' ) );
        echo '</div>';
        echo '</div>';
    ...
    */
    ...
    static function meta_box_import_export_settings( $post, $current_settings ) {
        ...
        <div class="save_settings">
            <img src="<?php echo $ppc_global_settings['folder_path'].'style/images/ajax-loader.gif'; ?>" title="<?php _e( 'Loading' , 'post-pay-counter'); ?>" alt="<?php _e( 'Loading' , 'post-pay-counter'); ?>" class="ajax_loader" id="ppc_import_settings_ajax_loader" />
            <input type="hidden" name="userid" value="<?php echo $current_settings['userid']; ?>" />
